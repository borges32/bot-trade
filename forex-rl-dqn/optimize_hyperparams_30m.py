#!/usr/bin/env python3
"""
Script de Otimização de Hiperparâmetros para USDJPY 30m.
...
class HyperparameterOptimizer:
    ...
    def define_search_space(self):
        """
        Define espaço de busca de hiperparâmetros para 30m.
        ...
        search_space = {
            ...
            'features': [
                ...
                {'use_ema': True, 'use_macd': False, 'use_rsi': False, 'use_bollinger': True, 'use_atr': True},
            ],
    ...
    def generate_combinations(self, max_combinations: int = 100):
        ...
        for features in search_space['features']:
            combo = {
                'features': features,
                'prediction_horizon': 10,  # padrão para 30m
                'learning_rate': 0.03,
                'num_leaves': 50,
                'max_depth': 6,
                'n_estimators': 500,
    ...
    def evaluate_combination(self, combination: dict, index: int, total: int) -> tuple:
        ...
        try:
            ...
            eval_result = {
                ...
                'train_direction_acc': metrics['train']['direction_accuracy'],
                
                # Métricas de validação
                'val_rmse': metrics['val']['rmse'],
                'val_mae': metrics['val']['mae'],
                'val_r2': metrics['val']['r2'],
                'val_direction_acc': metrics['val']['direction_accuracy'],
    ...
    def print_summary(self):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"RESUMO DA OTIMIZAÇÃO")
        logger.info(f"{'='*80}")
        ...
        logger.info(f"\nHiperparâmetros:")
        logger.info(f"  Prediction Horizon: {best['prediction_horizon']}")
    ...
    def _generate_detailed_report(self, best: Dict, top5: pd.DataFrame):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("RELATÓRIO DE OTIMIZAÇÃO - USDJPY 30m\n")
            ...
            f.write(f"   - Valor {int(best['prediction_horizon'])} = prevê {int(best['prediction_horizon']) * 30} minutos à frente\n\n")
            ...
            for idx, (_, row) in enumerate(top5.iterrows(), 1):
                ...
                f.write(f"   Horizon={int(row['prediction_horizon'])}, Subsample={row['subsample']}, "
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Otimização de hiperparâmetros para LightGBM - USDJPY 30m')
    parser.add_argument(
        '--config',
        type=str,
        default='config_hybrid_30m.yaml',
        help='Path para config base'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='optimization_results/usdjpy_30m',
