#!/usr/bin/env python3
"""
Script de Otimização de Hiperparâmetros para USDJPY 15m.
...
class HyperparameterOptimizer:
    ...
    def define_search_space(self):
        ...
        search_space = {
            ...
            # === PREDICTION HORIZON ===
            'prediction_horizon': [3, 5, 8],  # 45min, 75min, 2h
    ...
    def _generate_detailed_report(self, best: Dict, top5: pd.DataFrame):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("RELATÓRIO DE OTIMIZAÇÃO - USDJPY 15m\n")
            ...
            f.write(f"   - Valor {int(best['prediction_horizon'])} = prevê {int(best['prediction_horizon']) * 15} minutos à frente\n\n")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Otimização de hiperparâmetros para LightGBM - USDJPY 15m')
    parser.add_argument(
        '--config',
        type=str,
        default='config_hybrid_15m.yaml',
        help='Path para config base'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
        default='optimization_results/usdjpy_15m',
